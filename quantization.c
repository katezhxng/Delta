...
float * delta_encode(float * input, int length)
{
    float * delta = (float *) malloc (length * sizeof(float));

    float last = 0.0;
    for (int i = 0; i < length; i++)
    {
        float current = input[i];
        delta[i] = current - last;
        last = current;
    }
...
void delta_decode(float *buffer, int length)
{
    float last = 0.0;
    for (int i = 0; i < length; i++)
    {
...
int main() {
    ...
    float * input = readdata("Vf48.bin.f32", &length);
    ...
    //delta_encode(input, length);

    printf("after delta encoding: \n");
    ...
    printf ("length = %d, compressed_length = %d\n", length, compressed_length);
